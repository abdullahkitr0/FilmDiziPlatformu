...
$page_title = 'Son Eklenenler';
$page_description = 'En son eklenen film ve diziler';
...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="btn-group">
            <a href="<?= url('latest.php') ?>" class="btn btn-<?= $type ? 'outline-' : '' ?>primary">
                Tümü
            </a>
            <a href="<?= url('latest.php?type=movie') ?>" class="btn btn-<?= $type === 'movie' ? '' : 'outline-' ?>primary">
                Filmler
            </a>
            <a href="<?= url('latest.php?type=series') ?>" class="btn btn-<?= $type === 'series' ? '' : 'outline-' ?>primary">
                Diziler
            </a>
        </div>
    </div>
        ...
        <?php if ($total_pages > 1): ?>
            <nav class="mt-4">
                <ul class="pagination justify-content-center">
                    <?php if ($page > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="<?= url("latest.php?page=" . ($page - 1) . ($type ? "&type={$type}" : "")) ?>">
                    ...
                    if ($start > 1) {
                        echo '<li class="page-item"><a class="page-link" href="' . url("latest.php?page=1" . ($type ? "&type={$type}" : "")) . '">1</a></li>';
                        if ($start > 2) {
                    ...
                    for ($i = $start; $i <= $end; $i++) {
                        ...
                        echo '<a class="page-link" href="' . url("latest.php?page={$i}" . ($type ? "&type={$type}" : "")) . '">' . $i . '</a>';
                        echo '</li>';
                    }
                    ...
                    if ($end < $total_pages) {
                        ...
                        echo '<li class="page-item"><a class="page-link" href="' . url("latest.php?page={$total_pages}" . ($type ? "&type={$type}" : "")) . '">' . $total_pages . '</a></li>';
                    }
                    ?>
